...
对于分类, 缺失输入的分类和转录, *P* 往往是衡量模型的 **准确率(accuracy, 模型产生正确输出的比率, 或者用错误率(error rate)来度量)** . 我们通常把错误率称为 **0-1损失的期望** , 在一定特定的样本上, 如果结果是对的, 那么 0 -1 损失为0, 否则是1. 但是 0 - 1 损失模型或者准确率错误率对于密度估计这类任务而言, 是没有意义的.  
