...
所有除了第一个顶点和最后一个顶点可能相同之外其他顶点都是互异的, 称为 **简单路径**

**完全图** 图中每一对顶点间都存在一条边.

> 完全无向图的边数为 `!$\frac {1} {2} n (n - 1)$`

**顶点的度(记为 TD(v))** 和顶点相关联的边的数量. 对于有向图, 以顶点为头的弧的数量称为 **入度(记为ID(v))**, 以顶点为尾的弧的数量称为 **出度(记为OD(v))**, 所以 **度 = 入度 + 出度**.

> n个顶点的图的边数或者弧数 `!$e = \frac {1} {2} \sum_{i = 1} ^{n} TD(v_i)$`
