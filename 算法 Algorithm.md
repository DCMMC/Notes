...
对于有向图的 **弧(Arc/ 边edge)**, 有序对 **<v, w>** 表示从顶点 `!$v$` 到 `!$w$` 的一条弧, `!$v$` 被称为 **弧尾**, `!$w$` 被称为 **弧头** .
...
由于AOE网中有些活动可以并行进行, 所以完成工程的最短时间就是从开始点(入度为0的源点)到结束点(出度为0的汇点)的 **最长路径(权值之和最大的路径)的长度, 被称为关键路径**. 
...
事件发生的最晚时间(不影响整个工期的情况下能够发生的最晚的时间)和最早时间(最长路径长度)的差值为 **余量**.
