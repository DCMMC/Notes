...
* 有且仅有一个称为根的结点。
* 其余结点可以分为M个互不相交的子集合， 并且这些子集合中的每一个本身都是一颗树， 也称为根的子树。
...
2. 顺序存储结构: 把树假想补全为一颗满二叉树然后进行标号, 存储在一个一维数组中. 这种比较适用于完全二叉树.
...
1. n个结点的二叉树有 `!$n +1$` 个空链域, 并且这 `!$n + 1$` 个空链域 **最多** 有 `!$n$` 个被作为线索二叉树的线索(因为如果是一颗根节点左右子树都有的话, 有一个是遍历的最后一个(叶)结点, 肯定是没有后继的) (值得一提的是, 因为这 n + 1个空链域都可以被标志为了线索, 所以其实n个结点的二叉树含有的线索树为n + 1个)
2. 对于根节点左右字数都非空的二叉线索树, 空链域为1
3. 对于根节点没有左子树的二叉线索树, 空链域为2.

> 不过个人认为如果是带有 父结点指针 的三叉链表实现的二叉树的话, 没必要使用线索二叉树, 毕竟线索二叉树也只是对有空链域的结点方便找到前驱和后继一点, 并不是说整棵树所有结点都能直接找到前驱和后继, 不过对于二叉链表实现的二叉树, 线索话确实有利于充分利用空链域和帮助某些结点快速找到前驱和后继.

**实现**

想要线索化空链域, 必须在结点中增加两个标志位用来记录结点是否有左右子树, 如果左右线索标志为true的时候就说明该结点的左右链域指向前驱或者后继结点, 并且线索化就是相当于一次遍历, 并且在遍历的时候需要保存上一个被遍历到的结点(i.e. 前驱结点), 在发现
