...
构造器都是private的类不能用在别的类中用new创建，然而在别的类中却可以使用new创建这个类的数组（我知道为什么了：因为创建类的数组的时候并没有创建类的实例对象，类似于创建了C中的int ** pt；pt = （int **）malloc（n1*int *）；这里并没有创建int类型的数据成员）。
protected：继承访问权限：
