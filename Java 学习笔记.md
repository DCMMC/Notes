...
### (p)1.11.1 成员内部类(局部内部类)

只能从外部类的非静态方法（非静态方法可以直接new InnerClassName();）用new创建该外部类中的某个内部类的对象（不能在外部类的静态方法中直接用new创建内部类的对象, 因为new 该内部类的时候会自动隐式的调用this, 因为该内部类和它的外围对象相关联），最好指明这个对象的具体类型  i.e. OuterClassName.InnerClassName
默认只调用内部类，就算有同名的外部的类都没用，根本创建不了外部的同名类的对象了,外部的同名类直接被这个同名的内部类覆盖了
