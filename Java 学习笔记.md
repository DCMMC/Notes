...
要想从外部类的非静态方法（非静态方法可以直接new InnerClassName();）之外的任意位置用new创建某个内部类的对象（不能在外部类的静态方法中直接用new创建内部类的对象），必须具体（最好）指明这个对象的类型（为啥JDK8不用指明也是一样的。
默认只调用内部类，就算有同名的外部的类都没用，根本创建不了外部的同名类的对象了,外部的同名类直接被这个同名的内部类覆盖了） i.e. OuterClassName.InnerClassName
