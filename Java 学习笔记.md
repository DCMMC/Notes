...
super关键字：
可以在子类中覆盖基类中的方法和字段（签名和返回值要一模一样才会覆盖，而且权限知只能保持一样或者提升，而不能降低权限），这时候用super关键字（非静态的）（和this可以配合使用）(超类访问)访问基类中被替换的方法 e.g. super.methodName();
...
带参数的基类构造器在子类中调用必须显式的使用super关键字放在子类构造器的第一行。
代理：（有点像组合和继承的复合）（Java并不直接支持代理，不过JetBrains IDEA就支持这个）
...
正确清理：结合try子句和finally子句（后面会详细讲），避免使用finalize（）方法。
向上转型（upcast）：
...
类的加载：只有在创建类的第一个对象或者访问static域或static方法时才会加载类的class字节码，初次使用的时候也是static初始化的时候（按照定义类的书写顺序加载）
子类加载时会自动加载基类。
