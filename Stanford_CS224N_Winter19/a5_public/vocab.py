...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        self.char2id['<pad>'] = 0
        # (DCMMC) I don't know why use `{}` as start- and end-of-word as they are commonly-used.
        self.char2id['{'] = 1
        self.char2id['}'] = 2
        self.char2id['<unk>'] = 3
    ...
    def words2charindices(self, sents):
        ...
        return [[([self.start_of_word, ] + \
                   [self.char2id[c] if c in self.char2id else self.char_unk for c in word] + \
                   [self.end_of_word, ]) for word in sen] for sen in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        return torch.tensor(pad_sents_char(
            self.words2charindices(sents),
            self.char2id['<pad>']
        ), dtype=torch.long, device=device).permute(1, 0, 2)
