...
def pad_sents_char(sents, char_pad_token):
    ...
    return [[
        wd[:max_word_length] if len(wd) >= max_word_length else \
            (wd + [char_pad_token for _ in range(max_word_length - len(wd))]) for wd in sen] + \
        [[char_pad_token for _ in range(max_word_length)] for _ in range(max_len - len(sen))] for sen in sents]
