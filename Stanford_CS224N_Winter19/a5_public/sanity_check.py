...
def question_1g_sanity_check():
    ...
    print("Running test on a list of sentences")
    sentences = [['Human:', 'What', 'do', 'we', 'want?'], ['Computer:', 'Natural', 'language', 'processing!'], ['Human:', 'When', 'do', 'we', 'want', 'it?'], ['Computer:', 'When', 'do', 'we', 'want', 'what?']]
    input_tensor = vocab.to_input_tensor_char(sentences, torch.device('cpu'))
    assert list(input_tensor.shape) == [6, 4, 21]
...
def question_1h_sanity_check():
    ...
    with torch.no_grad():
        ...
        assert np.allclose(
            y2.numpy(), highway['x2_highway'].numpy()), 'The output is incorrect.'
    print("Sanity Check Passed for Question 1h: Module Highway!")
    print("-"*80)
