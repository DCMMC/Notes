...
若 `!$\alpha = v_0 \Rightarrow v_1 \Rightarrow \cdots \Rightarrow v_n = \beta$`, 则称 `!$\beta$` 可以由 `!$\alpha$` 推导出来。

**句型** 是由开始状态 `!$S$` 推导出来的（可能）含有非终结状态的串。
...
> 一般推导都是从最左到右替换非终结符(也就是 **最左推导** , 相反的就叫做 **最右推导**，**最右推导** 又称 **规范推导**， 左推导和右推导所得句型分别称为左句型和右句型（规范句型），不一定所有句型都有最右或最左推导)
