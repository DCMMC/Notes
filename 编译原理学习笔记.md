...
> 分析栈最开始只有一个开始符号，按照 LL(1) 表不断匹配当前状态栈栈顶状态和输入符号组成的条目，如果栈顶为当前输入符号，就匹配下一个输入符号直到状态栈到达底部。
...
==`!$GOTO(I, X)$`== 定义为项集 `!$I$` 中所有形如 `!$A \rightarrow \alpha \cdot X \beta$` （X 为文法符号，终结或者非终结符号）的项对应的项 `!$A \rightarrow \alpha X \cdot \beta$` 的集合的闭包。

文法 G\[S] 的 ==LR(0)== 自动机的开始状态就是 ==Closure(`!$[S' \rightarrow \cdot S]$`)==, 如果在当前正处于状态 `!$j$`，如果下一个输入符号为 `!$a$` 且状态 `!$j$` 有一个在 `!$a$` 的转换，就移入a，否则按照当前所在状态的项集中选择一个产生式进行归约。
...
![enter description here](./images/1529291109629.png)

归约就是将相应的产生式的右部压入弹出符号栈，将左部压入符号栈，并且一个一个将状态栈的栈顶DFA状态弹出，直到在新的栈顶状态寻找到一个在符号栈栈顶的转换（e.g., 上例中第 3 行，按照 `!$F \rightarrow id$`  归约后，符号栈顶为 `!$F$`, 这时候状态栈栈顶为 0 （5号状态弹出来了），状态 0 有一个接收 F 到状态 3 的转换，所以进入到状态 3；再 e.g. 第 7 行，连续弹出了 10，7，2才找到可以接受 T 到 2）。

> 上述过程具体可以使用格局（configuration）`!$(s_0 s_1 \cdots s_m, a_i a_{i + 1} \cdots a_n \$)$` 左边式状态栈的内容， 右边为余下的输入

// TODO 比较繁琐， 见龙书第二版（中文） P159

> 上述的自动机又叫做 **识别 G\[S] 所有活前缀（可行前缀）的 DFA**

// TODO 具体的证明上述观点见龙书第二版（中文）P163 (p)4.6.5

**LR 语法分析表**

==ACTION== 表中的四种格局：

* `!$si$` 表示 **移入**， 将状态 `!$i$` 压入状态栈
* `!$rj$` 表示**归约**， 将产生式 `!$j$` （**需要对产生是进行编号** ）进行归约
* `!$acc$` 表示接受
* 空白表示出错

所有的 LR 文法（SLR， LR,  LALR 这些）都用上面描述的语法分析表。

==ACTION== 条目用于确定对当前输入是执行移入还是归约还是出错处理

==GOTO== 条目（请区别开 Goto 函数）用于归约 `!$A \rightarrow \beta$` 的时候执行连续弹出状态栈中的状态（ **弹出 `!$|\beta|$` 个状态** ）

==GOTO== 表就是用于 ==LR(0)== 自动机中接受非终结符的转换。

**LR(0) 语法分析表**

* ==ACTION== 表中的移入和 ==GOTO== 表直接看 ==LR(0)== 自动机的状态转移就行了
* 若归约项 `!$[A \rightarrow \alpha \cdot]$`（如果该产生式的标号为 `!$j$`） 属于项集 `!$I_i$`，那么 ==`!$ACTION[i, :] = r_j$`==(==:== 表示ACTION 表中该行的所有条目，也就是终结符和右界定符 ==$== 或 ==#== )
* 空白的条目均表示出错
* 接受 `!$[S' \rightarrow S \cdot]$` 属于 `!$I_i$`, 则 `!$ACTION[i, \$] = acc$`

> 上述过程中任何一个条目出现了冲突(i.e., 存在一个条目中有两个元素的情况)，那就说民这个文法不是 LR(0) 的

**SLR(1) 语法分析表**

除了处理 `!$r_j$` 和 ==LR(0)== 分析表不一样之外，其他的都一样。

* 若归约项 `!$[A \rightarrow \alpha \cdot]$`（如果该产生式的标号为 `!$j$`） 属于项集 `!$I_i$`，那么 `!$[ACTION[i, a] = r_j \text{ for a in } FOLLOW(A)]$` (==Python== 风格列表生成式，也就是说不是该行所有条目都填充 `!$r_j$`，而是所有在 ==FOLLOW(A)== 中的条目)

> 上述过程中任何一个条目出现了冲突(i.e., 存在一个条目中有两个元素的情况)，那就说民这个文法不是 SLR(1) 的

**LR(1)** 语法分析表

// TODO 不考，先放着，==人生苦短，用自动化构造工具多好==
