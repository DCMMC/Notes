...
归约就是将相应的产生式的右部压入弹出符号栈，将左部压入符号栈，并且将栈的栈顶DFA状态弹出，在新的栈顶状态寻找一个在符号栈栈顶的转换（e.g., 上例中第 3 行，按照 `!$F \rightarrow id$`  归约后，符号栈顶为 `!$F$`, 这时候状态栈栈顶为 0 （5号状态弹出来了），状态 0 有一个接收 F 到状态 3 的转换，所以进入到状态 3。
