...
设 `!$\alpha, \delta \in V^*, \beta \in V^+$` 是文法 `!$G[S]$` 的句型，若对于 `!$A \in V_N$` 有 `!$S \overset{*}{\Rightarrow} \alpha A \delta, A \overset{+}{\Rightarrow} \beta$` 则称 `!$\beta$` 是句型 `!$\alpha \beta \bdelta$` 的短语（如果 `!$A \Rightarrow \beta$` 则称直接短语（
