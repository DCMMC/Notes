...
AF (auxiliary carry flag) 辅助进位标志, 记录运算时第3位(半个字节, **位数是从第0位开始的**)产生的进位值
...
局部数据 | DS | 除相对于堆栈以及串处理指令的目的串以外的所有数据访问
目的串 | ES | 串处理指令的目的串
...
> 注意: 如果两个操作符是r或者m的话, 长度要一致, 并且不能同时为m
...
> 注意: 因为商都是被除数的长度的一半, 所以如果 被除数的高8位(高16位, i.e. DX)的绝对值如果大于除数, 就会引起商溢出, 直接触发 `0` 型中断.
...
将OPR的值设置为它的二进制位向右移动CNT位, 并且所有的高位用0补齐, 右边最后一个超出边界的位放入CF中.
...
> 上述指令中, 除了CF按照各条指令规定设置, **OF为只有当CNT= 1时, 在移位后最高有效位发生变化时, OF为1**. 循环移位指令不影响除了CF和OF外的其他标志位,
> 移位指令则根据移位后的结果设置SF, ZF和PF位.

## 控制转移指令

### 无条件转移指令

JMP 指令, 具体看前面与转移地址有关的寻址方式

### 有条件转移指令

有条件转移只能用短转移格式(也就是一个8bit的偏移量, 在80386之后, 还可以用近转移, 也就是一个32bit的偏移量)

* 条件: ZF = 1, 指令: JZ (jump if zero, 结果为0) JE(jump if equal)
...
> 符号地址在使用的时候就等价于EA, 例如 上面的ARRAY的偏移地址是 0005H, 在汇编中使用 ARRAY就等价于使用 [0005H] (记住这里是有中括号的, 也就是取内存中的值)
