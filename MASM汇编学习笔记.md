...
> 以上三个命令都可作字或者字节运算(80386+为双字), 而且 **除了INC不
> 影响CF标志外, 它们都影响条件标志位(CF进位, ZF结果为0, SF符号, OF溢出)**
...
> 对于存储器寻址中含有符号地址的情况 例如 ARRAY[SI] 就相当于 [ARRAY][SI], 编译器( **in my case, MASM5** ) 会直接报错: **cannot address with segment register**, 这时候需要在前面强制加入段前缀: e.g. DS: ARRAY[SI] (等价 DS: [ARRAY][SI], 也就是等价 DS: [ARRAY + SI], 不过仍然会在双操作数指令并且 ARRAY和 另外一个操作数需要的长度不一样的时候会警告(ARRAY短于另一操作数)或报错(ARRAY长于另一操作数), 这时候推荐使用 **OFFSET ARRAY[SI]** 这种写法, 这样可以编译器可以根据 SI来确定长度
